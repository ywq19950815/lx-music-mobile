...
async function main() {
  ...
  // 点击 Tab 3（排行榜，x: 215, y: 846）
  console.log('Clicking Tab 3 (排行榜)...')
  await ws.send('Input.dispatchMouseEvent', { type: 'mousePressed', x: 215, y: 846, button: 'left', clickCount: 1 })
  await ws.send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: 215, y: 846, button: 'left', clickCount: 1 })
  await sleep(1200)
  ...
  console.log('Visible Texts on Tab 3:', texts.result.value)
  ...
  fs.writeFileSync('preview/shot_tab_3_gallery.png', Buffer.from(shot.data, 'base64'))
  console.log('Saved preview/shot_tab_3_gallery.png')
