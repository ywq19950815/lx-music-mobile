...
async function main() {
  ...
  const err = await ws.send('Runtime.evaluate', {
    expression: `(() => {
      import('../src/app').catch(e => {
  ...
  console.log('Direct import error:', err)

  // 抓取 DOM 中的 pre 标签内容
  const pre = await ws.send('Runtime.evaluate', {
    expression: `(() => {
      const p = document.querySelector('pre')
      return p ? p.innerText : 'no pre'
    })()`,
    returnByValue: true
  })
  console.log('Pre content:', pre.result.value)
