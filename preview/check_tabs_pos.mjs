...
async function main() {
  ...
  // 检查当前 DOM 里的 Tab 元素
  const tabs = await ws.send('Runtime.evaluate', {
    expression: `(() => {
      const phone = document.getElementById('phone')
      const r = phone ? phone.getBoundingClientRect() : null
      return {
        phoneRect: r ? { x: r.x, y: r.y, w: r.width, h: r.height, top: r.top, bottom: r.bottom } : null,
  ...
  console.log('Phone Rect info:', tabs.result.value)
