...
async function main() {
  ...
  // 点击第 5 个 Tab（齿轮设置图标：x: 362.59, y: 846）
  console.log('Dispatching click to Setting Tab...')
  await ws.send('Input.dispatchMouseEvent', { type: 'mousePressed', x: 362, y: 846, button: 'left', clickCount: 1 })
  await ws.send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: 362, y: 846, button: 'left', clickCount: 1 })

  await sleep(1000)

  // 截取当前界面！
  await shot('setting_page_after_real_click.png')

  // 诊断当前显示的可见区域文本
  const visibleText = await evalJs(`(() => {
    // 找出在屏幕可见矩形内的文字
    ...
    return inView.map(e => e.innerText).join(' | ').slice(0, 500)
  })()`)
