...
function httpGetJson(p, method = 'GET') {
  return new Promise((resolve, reject) => {
    const req = http.request({ host: '127.0.0.1', port: DEBUG_PORT, path: p, method }, (res) => {
      let buf = ''
      res.on('data', (d) => { buf += d })
...
async function main() {
  ...
  // 2. 切换到“我的音乐” Tab（测试去 Neo 化的歌单卡片与纯净白表面）
  await evalJs(`(async () => {
    try {
  ...
  await sleep(1500)
  await shot('shot_02_mylist_modern.png')
  ...
  await sleep(1500)
  await shot('shot_03_leaderboard_modern.png')

  // 4. 切换到“设置”页面（查看柔和卡片、版本、关于与优质音源库卡片）
  await evalJs(`(async () => {
    try {
      const commonState = await import('/@fs/E:/Desktop/AI/lx-music-mobile/src/store/common/state.ts')
  ...
  await sleep(2000)
  await shot('shot_04_setting_modern.png')

  // 5. 切换回首页并展开播放详情页（黑胶唱机系统）
  await evalJs(`(async () => {
    try {
      const commonState = await import('/@fs/E:/Desktop/AI/lx-music-mobile/src/store/common/state.ts')
      commonState.default.navActiveId = 'nav_search'
    } catch (e) {}
  })()`)
  await sleep(1000)
  await evalJs(`(() => {
  ...
  await sleep(2000)
  await shot('shot_05_player_modern.png')
