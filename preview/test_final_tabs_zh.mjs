...
async function main() {
  ...
  // 1. 设置 localStorage 的 @setting_v1 中 common.langId 为 zh_cn
  console.log('1. Correcting localStorage to zh_cn...')
  await evalJs(`(() => {
    try {
      const s = localStorage.getItem('@setting_v1')
      if (s) {
        const obj = JSON.parse(s)
        obj['common.langId'] = 'zh_cn'
        localStorage.setItem('@setting_v1', JSON.stringify(obj))
      }
    } catch (e) {}
  })()`)

  // 2. 刷新页面以应用最新的 zh_cn 语言
  ...
  const langCheck = await evalJs(`(() => {
    return {
      locale: globalThis.i18n ? globalThis.i18n.locale : 'undefined',
      t_search: globalThis.i18n ? globalThis.i18n.t('nav_search') : '',
      t_songlist: globalThis.i18n ? globalThis.i18n.t('nav_songlist') : '',
      t_top: globalThis.i18n ? globalThis.i18n.t('nav_top') : '',
      t_love: globalThis.i18n ? globalThis.i18n.t('nav_love') : '',
      t_setting: globalThis.i18n ? globalThis.i18n.t('nav_setting') : ''
    }
  })()`)
  console.log('Language Check:', JSON.stringify(langCheck, null, 2))
  ...
  // 底部 TabBar 的 5 个 Tab 对应 x 轴中心，y 轴在底部
  const tabY = Math.round(phoneRect.y + phoneRect.height - 30)
  const tabStep = phoneRect.width / 5
  const tabXs = [0, 1, 2, 3, 4].map(i => Math.round(phoneRect.x + tabStep * i + tabStep / 2))

  const tabDefs = [
    { name: '1_search', title: '搜歌', id: 'nav_search', x: tabXs[0] },
    { name: '2_songlist', title: '歌单', id: 'nav_songlist', x: tabXs[1] },
    { name: '3_top', title: '排行榜', id: 'nav_top', x: tabXs[2] },
    { name: '4_love', title: '我的音乐', id: 'nav_love', x: tabXs[3] },
    { name: '5_setting', title: '设置', id: 'nav_setting', x: tabXs[4] }
  ]

  const outputFiles = []

  for (const tab of tabDefs) {
    console.log(`\nTesting Tab ${tab.name} (${tab.title}) click at (${tab.x}, ${tabY})...`)
    ...
    const visibleTexts = await evalJs(`(() => {
      ...
      return Array.from(new Set(inView)).slice(0, 12)
    })()`)
    console.log(`  Visible Texts on ${tab.title}:`, visibleTexts)
    ...
    const outPath = path.resolve(`preview/gallery_tab_${tab.name}.png`)
    fs.writeFileSync(outPath, Buffer.from(shot.data, 'base64'))
    console.log(`  Saved screenshot: ${outPath}`)
    outputFiles.push(outPath)
  }

  console.log('\nAll 5 tabs tested and screenshotted successfully!')
  ws.close()
}
