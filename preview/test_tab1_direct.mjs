...
async function main() {
  ...
  // 查看当前激活的 navActiveId
  const currentNav = await ws.send('Runtime.evaluate', {
    expression: `(() => {
      // 触发 setNavActiveId('nav_search')
      import('/src/core/common.ts').then(m => {
        m.setNavActiveId('nav_search')
      })
      return 'triggered'
    })()`,
    awaitPromise: true,
  ...
  console.log('Nav triggered:', currentNav.result.value)
  await sleep(1500)
