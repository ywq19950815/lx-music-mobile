...
async function main() {
  ...
  // 诊断当前 DOM
  const diagnosis = await evalJs(`(() => {
    const phone = document.getElementById('phone')
    const root = document.getElementById('root')
    
    // 找出所有带 transform 的元素
    const all = Array.from(document.querySelectorAll('*'))
    const transformElements = all
      .filter(el => {
    ...
    // 检查 commonState.navActiveId
    let navActiveId = 'unknown'
    try {
      navActiveId = window.global?.lx?.navActiveId || 'check-state'
    } catch {}

    return {
      phoneRect: phone ? phone.getBoundingClientRect() : null,
      transformElements,
      activeTitleText: document.querySelector('h1, [style*=\"font-size: 22\"], [style*=\"fontSize: 22\"]')?.innerText,
    }
  })()`)
