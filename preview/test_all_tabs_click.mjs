...
async function main() {
  ...
  const getVisibleText = async () => {
    return await evalJs(`(() => {
      ...
      return inView.map(e => e.innerText).join(' | ').slice(0, 200)
    })()`)
  }
  ...
  const tabs = [
    { name: '1_Search', x: 67, y: 846 },
    { name: '2_Songlist', x: 141, y: 846 },
    { name: '3_Leaderboard', x: 215, y: 846 },
    { name: '4_Mylist', x: 288, y: 846 },
    { name: '5_Setting', x: 362, y: 846 },
  ]

  for (const tab of tabs) {
    console.log(`\n=== Testing Click Tab: ${tab.name} ===`)
    await ws.send('Input.dispatchMouseEvent', { type: 'mousePressed', x: tab.x, y: tab.y, button: 'left', clickCount: 1 })
    await ws.send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: tab.x, y: tab.y, button: 'left', clickCount: 1 })
    await sleep(600)
    const text = await getVisibleText()
    console.log(`Visible in ${tab.name}:`, text)
    await shot(`tab_click_${tab.name}.png`)
  }
